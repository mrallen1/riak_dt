...
value({Active, _Tombstone}) ->
    [{K, Type:value(V)} || {{_Name, Type}=K, {_VClock, V}} <- orddict:to_list(Active)].
...
apply_ops([], _Actor, Map) ->
    Map;
apply_ops([{update, {_Name, Mod}=Key, Op} | Rest], Actor, {Active, Tombstones}) ->
    InitialValueClock =  riak_dt_vclock:increment(Placeholder, riak_dt_vclock:fresh()),
    InitialValue = Mod:update(Op, Actor, ModLnew()),
    Initial = {InitialValueClock, InitialValue},
    NewDict = orddict:update(Key, update_fun(Mod, Op, Actor), Initial, Dict0),
    {Actors, Dict};
apply_ops([{remove, {_Name, Mod}=Key}, _Actor, {Active, Tombestones}) ->
...
update_fun(Mod, Op, Actor) ->
    fun({Clock, V}) ->
            {riak_dt_vclock:increment(Actor, Clock), Mod:update(Op, Actor, V)}
    end.
